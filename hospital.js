...
app.put('/update',middleware.checkToken,(req,res)=>{
    ...
    console.log(myquery);
    var myquery1={vid:myquery};
    db.collection("VentilatorDetails").updateOne(myquery1,{$set:{"status": status}},function(err,obj){
        if(err) throw err;
...
app.post('/insert',middleware.checkToken,(req,res)=>{
    ...
    console.log(myquery);
    db.collection("VentilatorDetails").insertOne(myquery,function(err,obj){
        if(err) throw err;
